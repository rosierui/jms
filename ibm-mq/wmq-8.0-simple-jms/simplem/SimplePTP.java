...
public class SimplePTP {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      cf.setStringProperty(WMQConstants.WMQ_CHANNEL, "JAVA.CHANNEL");// ! "SYSTEM.DEF.SVRCONN
      cf.setIntProperty(WMQConstants.WMQ_CONNECTION_MODE, WMQConstants.WMQ_CM_CLIENT);
      cf.setStringProperty(WMQConstants.WMQ_QUEUE_MANAGER, "QM1");
      cf.setStringProperty(WMQConstants.WMQ_APPLICATIONNAME, "SimplePTP (JMS)");

      // Create JMS objects
      connection = cf.createConnection("mq_user", "Test1");
      session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
      destination = session.createQueue("queue:///REQUEST_Q");
